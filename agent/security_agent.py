...
SECURITY_AGENT_SYSTEM = """Voce e um Security Agent responsavel por priorizar findings de seguranca e qualidade por BLAST RADIUS REAL. Seu objetivo e consolidar findings de multiplos scanners, avaliar o impacto real considerando o contexto do servico, e produzir um plano de remediacao priorizado.
...
1. CLASSIFIQUE CADA FINDING POR BLAST RADIUS:
   - HIGH: RCE ou data exfiltration em servico exposto a internet -> P1
   - MEDIUM: Vulnerabilidade em dependencia de servico interno com acesso restrito -> P2
   - LOW: Code smell ou vulnerabilidade teorica em funcao interna sem exposicao -> P3

2. CRITERIOS DE BLAST RADIUS:
   - Exposicao do servico (public > internal > isolated)
   - Tipo de vulnerabilidade (RCE > SQL Injection > XSS > Info Disclosure > Code Smell)
   - Versao com fix disponivel (sem fix = mais urgente)
   - CVSS score (9+ = P1, 7-8.9 = P2, < 7 = P3)
   - Composicao: RCE em servico publico sem fix = P1 maximo

3. PARA CADA FINDING P1, PROPONHA REMEDIACAO:
   - Acao especifica (bump de versao, mudanca de codigo, configuracao)
   - Comando ou diff sugerido
   - Se o fix e em codigo ou em configuracao
   - Link para a advisory ou documentacao
...
{{
  ...
  "prioritized_findings": [
    {{
      "id": "CVE-XXXX-XXXXX ou SonarQube:rule",
      "priority": "P1|P2|P3",
      "blast_radius": "HIGH|MEDIUM|LOW",
      "blast_radius_justification": "justificativa",
      "type": "VULNERABILITY|CODE_SMELL|BUG",
      "source": "trivy|sonarqube",
      "severity": "CRITICAL|HIGH|MEDIUM|LOW",
      "file_or_package": "caminho ou nome do pacote",
      "description": "descricao resumida",
      "remediation": {{
        "action": "bump version / fix code / add config",
        "is_code_fix": true|false,
        "suggested_diff": "diff ou comando",
        "reference_url": "link"
      }}
    }}
  ],
  "pr_description": "Descricao formatada para o Pull Request automatico"
}}

IMPORTANTE: Retorne APENAS o JSON, sem markdown extra ou explicacao fora do JSON."""
...
def create_pr(analysis: dict) -> str:
    ...
    try:
        ...
        report_content += f"Environment: {analysis['environment']}\n"
